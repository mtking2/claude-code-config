...
#   - iTerm2 window title
#   - Warp terminal (uses WARP_IS_LOCAL_SHELL_SESSION)
#   - X11 window title (Linux)
#   - Wayland/Sway window title
#
# EXAMPLES
#   # Send notification
...
NTFY_SERVER=$(yq -r '.ntfy_server' "$CONFIG_FILE" 2>/dev/null || echo "https://ntfy.sh")
...
get_terminal_title() {
    ...
    if [[ "${TERM_PROGRAM:-}" == "tmux" ]] && command -v tmux >/dev/null 2>&1; then
        ...
        # macOS: Get Terminal, iTerm2, or Warp window title
        if [[ "${TERM_PROGRAM:-}" == "iTerm.app" ]]; then
            title=$(osascript -e 'tell application "iTerm2" to name of current window' 2>/dev/null || echo "")
        elif [[ "${TERM_PROGRAM:-}" == "WarpTerminal" ]] || [[ "${WARP_IS_LOCAL_SHELL_SESSION:-}" == "1" ]]; then
            # Warp terminal detection
            # Warp sets WARP_IS_LOCAL_SHELL_SESSION=1 and uses a different window title format
            # Try to get the window title from Warp
            title=$(osascript -e 'tell application "Warp" to name of front window' 2>/dev/null || echo "")
            # If that doesn't work, use the session name or working directory
            if [[ -z "$title" ]] || [[ "$title" == "Warp" ]]; then
                # Warp often shows just "Warp" as the window title, so use pwd instead
                title="Warp: $(basename "$PWD")"
            fi
        else
